---
name: adr-skill
description: Use when you need to propose, write, update, accept/reject, deprecate,
  or supersede an ADR; bootstrap an adr folder and index; consult existing ADRs before
  implementing changes; or enforce ADR conventions. Creates and maintains Architecture
  Decision Records optimized for agentic coding workflows using Socratic questioning
  and agent-readiness validation.
metadata:
  category: technique
  triggers:
...
`bootstrap_adr.js`) require Node.js. If Node is unavailable in the current
environment, **do not skip ADR creation** — use the manual fallback in
[`ADR-TEMPLATE.md`](ADR-TEMPLATE.md) instead. It documents the same
directory-detection, filename, and template steps the scripts automate, plus
a copy-pasteable ADR template with the same fields the scripts would
generate. Every "Preferred: run `scripts/...`" instruction below has an
equivalent manual step in `ADR-TEMPLATE.md`.
