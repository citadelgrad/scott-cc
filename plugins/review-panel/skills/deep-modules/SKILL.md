...
name: deep-modules
description: >-
  Use when a module's interface has too many parameters or methods, when there
  are too many small classes each doing too little, or when methods just forward
  calls to other methods. Measures module depth — whether the interface is
  simple relative to the implementation behind it. Not for adjacent-layer
  abstraction quality (use abstraction-quality) or merge/split decisions (use
  module-boundaries).
